I'm a backend software engineer with 10+ years of experience across backend systems, cloud infrastructure, automation, and data systems. These days, I'm especially interested in Rust, security, and open-source work.

**Outside of tech**, I enjoy hiking, mountains, Brazilian Jiu-Jitsu, and spending time with my dog Ahri, a Border Collie.

Full first name: Volodymyr — Vova or **Vlad for short**.

---

- **Backend engineering**: TypeScript, Python
- **Rust**: working knowledge, actively improving
- **Security**: growing focus on security, including offensive security and security-minded engineering
- **Cloud**: AWS, some GCP
- **Infrastructure**: Kubernetes, ArgoCD, Docker, Terraform  

Additional experience:

- **Data systems**: enterprise projects with Airflow, Spark, PostgreSQL, AWS Glue, Redshift, RDS  
- **Blockchain / Web3**: solid Polkadot background, Ethereum basics
- **Java**: working knowledge, used occasionally in smaller projects  
- **Frontend / desktop**: React, Angular, Tauri for small MVPs and internal tools

---

- **CompTIA**: [Security+](https://cp.certmetrics.com/CompTIA/en/public/verify/credential/5271902b8d4c4848a0d61eb37048e0db)
- **Cloud Native Computing Foundation**: [Certified Kubernetes Application Developer](https://ti-user-certificates.s3.amazonaws.com/e0df7fbf-a057-42af-8a1f-590912be5460/8c5e0272-2e2f-4c8b-862f-403dee2fd4a3-volodymyr-brazhnyk-8c42c73a-a600-4a96-be0e-9d3dc11da6d6-certificate.pdf)
- **AWS**: Certified Big Data - Specialty (W2XSKNPCMEQ41H3L)
- **AWS**: Certified Solutions Architect - Associate (3DNNM01KN1B1Q7SQ)
- **Oracle**: [Certified Professional Java SE 17 Developer](https://catalog-education.oracle.com/pls/certview/sharebadge?id=FA07B7C0D0A924EBA90E0961061631E25489ED6FCB48A3DFAD2319E7C60CB26E)
